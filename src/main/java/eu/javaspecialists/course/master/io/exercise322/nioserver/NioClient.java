...
public class NioClient implements Runnable {
    ...
    private final Map<SocketChannel, RspHandler> rspHandlers = new ConcurrentHashMap<SocketChannel, RspHandler>();
    ...
    public void send(byte[] data, RspHandler handler) throws IOException {
        ...
        Queue<ByteBuffer> newQueue = new ConcurrentLinkedQueue<ByteBuffer>();
        Queue<ByteBuffer> queue = this.pendingData.putIfAbsent(socket, newQueue);
        if (queue == null)
            queue = newQueue;
    ...
    private void write(SelectionKey key) throws IOException {
        ...
        if (queue.isEmpty()) {
            // We wrote away all data, so we're eu longer interested
